    ...
    <div class="row">
        <div class="col-12 d-flex justify-content-center">
            <style>
                ...
                .reviews-carousel-slide.is-active .review-card {
                    animation: reviewsCardEnter 0.45s ease both;
                }

                .reviews-carousel-slide.is-active .review-card:nth-child(2) {
                    animation-delay: 0.05s;
                }

                .reviews-carousel-slide.is-active .review-card:nth-child(3) {
                    animation-delay: 0.1s;
                }

                @keyframes reviewsCardEnter {
                    from {
                        opacity: 0;
                ...
                .review-avatar.no-photo .review-avatar-fallback,
                .review-avatar.fallback-active .review-avatar-fallback {
                    display: flex;
                }
                ...
                .reviews-write-btn {
                    background: linear-gradient(135deg, #fbbc04 0%, #fca903 100%);
                    color: #333;
                    border: none;
                    padding: 10px 24px;
                    border-radius: 50px;
                ...
                .reviews-write-btn:hover {
                    ...
                    box-shadow: 0 6px 18px rgba(251, 188, 4, 0.35);
                    color: #333;
                    text-decoration: none;
                }
            ...
            <div id="google-reviews-container" style="width: 100%; max-width: 1200px;">
                <div id="google-reviews-status" style="text-align: center; font-size: 1.05rem; color: #5f6368; padding: 30px 20px;">
                    <?php echo __('reviews.loading'); ?>
                </div>
                <div class="reviews-stats-bar" id="google-reviews-stats-bar" style="display: none;">
                    <div class="reviews-stats-left">
                        <div class="reviews-rating-inline">
                            <div class="reviews-stars-inline" id="reviews-avg-stars"></div>
                            <span class="reviews-avg-value" id="reviews-avg-value">0.0</span>
                        </div>
                        <span class="reviews-count-text"><span class="reviews-count-number" id="reviews-total-count">0</span> <?php echo __('reviews.total'); ?></span>
                    </div>
                    <a href="https://g.page/r/CX9a_UXY06_QEBM/review" target="_blank" rel="noopener noreferrer" class="reviews-write-btn">
                        ✎ <?php echo __('reviews.write_review'); ?>
                    </a>
                </div>
                <div class="reviews-carousel-wrapper" id="google-reviews-carousel-wrapper" style="display: none;">
                    <div id="google-reviews-list" class="reviews-carousel-track"></div>
                </div>
                <div class="reviews-carousel-controls" id="google-reviews-controls" style="display: none;">
                    <button type="button" id="google-reviews-prev" class="reviews-carousel-btn" aria-label="Anterior">&#10094;</button>
                    <span id="google-reviews-indicator" class="reviews-carousel-indicator">1 / 1</span>
                    <button type="button" id="google-reviews-next" class="reviews-carousel-btn" aria-label="Siguiente">&#10095;</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    const translations = {
        ...
        reviews: {
            ...
            loading: <?php echo json_encode(__('reviews.loading')); ?>,
            no_reviews: <?php echo json_encode(__('reviews.no_reviews')); ?>,
            error_loading: <?php echo json_encode(__('reviews.error_loading')); ?>,
            guest: <?php echo json_encode(__('reviews.guest')); ?>,
            updated_on: <?php echo json_encode(__('reviews.updated_on')); ?>
        }
    };
    ...
    let googleReviewsAutoplayTimer = null;
    let googleReviewsTouchStartX = 0;
    let googleReviewsTouchStartY = 0;
    const googleReviewsAutoplayDelay = 7000;
    ...
    function formatReviewDate(rawDate) {
        ...
        const locale = translations.locale || 'es-ES';
    ...
    function renderReviewStars(rating) {
        ...
        let stars = '';
        for (let i = 1; i <= 5; i++) {
            stars += `<span style="color:${i <= safeRating ? '#fbbc04' : '#d7d7d7'}; font-size: 1.1rem;">&#9733;</span>`;
        }
        return stars;
    }
    ...
    function splitReviewsIntoSlides(reviews, perSlide) {
        const slides = [];
    ...
    function updateReviewsCarouselUI() {
        const trackEl = document.getElementById('google-reviews-list');
        const prevBtn = document.getElementById('google-reviews-prev');
        const nextBtn = document.getElementById('google-reviews-next');
        const indicatorEl = document.getElementById('google-reviews-indicator');

        if (!trackEl || !prevBtn || !nextBtn || !indicatorEl) {
            return;
        }

        const totalSlides = Number(trackEl.dataset.totalSlides || 1);
        if (googleReviewsSlideIndex >= totalSlides) {
            googleReviewsSlideIndex = totalSlides - 1;
        }
        if (googleReviewsSlideIndex < 0) {
            googleReviewsSlideIndex = 0;
        }

        trackEl.style.transform = `translateX(-${googleReviewsSlideIndex * 100}%)`;
        const allSlides = trackEl.querySelectorAll('.reviews-carousel-slide');
        for (let i = 0; i < allSlides.length; i++) {
            if (i === googleReviewsSlideIndex) {
                allSlides[i].classList.add('is-active');
            } else {
                allSlides[i].classList.remove('is-active');
            }
        }
        indicatorEl.textContent = `${googleReviewsSlideIndex + 1} / ${totalSlides}`;
        prevBtn.disabled = googleReviewsSlideIndex <= 0;
        nextBtn.disabled = googleReviewsSlideIndex >= totalSlides - 1;
    }

    function stopReviewsAutoplay() {
        if (googleReviewsAutoplayTimer) {
            clearInterval(googleReviewsAutoplayTimer);
            googleReviewsAutoplayTimer = null;
        }
    }
    ...
    function startReviewsAutoplay() {
        ...
        const totalSlides = Number(trackEl.dataset.totalSlides || 1);
        if (totalSlides <= 1 || document.hidden) {
            stopReviewsAutoplay();
            return;
        }

        stopReviewsAutoplay();
        googleReviewsAutoplayTimer = setInterval(function() {
            const dynamicTotalSlides = Number(trackEl.dataset.totalSlides || 1);
            if (dynamicTotalSlides <= 1 || document.hidden) {
                return;
            }
            googleReviewsSlideIndex = (googleReviewsSlideIndex + 1) % dynamicTotalSlides;
            updateReviewsCarouselUI();
        }, googleReviewsAutoplayDelay);
    }

    function restartReviewsAutoplay() {
        stopReviewsAutoplay();
        startReviewsAutoplay();
    }

    function moveReviewsSlide(step) {
        const trackEl = document.getElementById('google-reviews-list');
        if (!trackEl) {
            return;
        }
        ...
        updateReviewsCarouselUI();
        restartReviewsAutoplay();
    }

    function attachReviewsCarouselEvents() {
        const prevBtn = document.getElementById('google-reviews-prev');
        const nextBtn = document.getElementById('google-reviews-next');
        const wrapperEl = document.getElementById('google-reviews-carousel-wrapper');
        ...
        if (wrapperEl && !wrapperEl.dataset.bound) {
            wrapperEl.addEventListener('mouseenter', stopReviewsAutoplay);
            wrapperEl.addEventListener('mouseleave', startReviewsAutoplay);

            wrapperEl.addEventListener('touchstart', function(event) {
                if (!event.touches || event.touches.length === 0) {
                    return;
                }
                googleReviewsTouchStartX = event.touches[0].clientX;
                googleReviewsTouchStartY = event.touches[0].clientY;
                stopReviewsAutoplay();
            }, { passive: true });
            ...
            wrapperEl.addEventListener('touchend', function(event) {
                ...
                const deltaX = touchEndX - googleReviewsTouchStartX;
                const deltaY = touchEndY - googleReviewsTouchStartY;

                if (Math.abs(deltaX) > 45 && Math.abs(deltaX) > Math.abs(deltaY)) {
                    moveReviewsSlide(deltaX > 0 ? -1 : 1);
                } else {
                    startReviewsAutoplay();
                }
            }, { passive: true });

            wrapperEl.dataset.bound = '1';
        }

        if (!document.body.dataset.reviewsVisibilityBound) {
            ...
            document.body.dataset.reviewsVisibilityBound = '1';
        }
    }

    function buildReviewCard(review) {
        const author = escapeHtml(review.author_name || translations.reviews.guest);
        const authorInitial = (author.charAt(0) || 'G').toUpperCase();
        const photo = escapeHtml(review.author_photo_url || '');
        const comment = escapeHtml(review.comment || '');
        const dateText = formatReviewDate(review.created_at);
        const avatarClass = photo ? 'review-avatar has-photo' : 'review-avatar no-photo';
        const avatarImage = photo
            ? `<img src="${photo}" alt="${author}" loading="lazy" referrerpolicy="no-referrer" onerror="this.style.display='none'; this.parentNode.classList.add('fallback-active');">`
            : '';
            ...
            <article class="review-card">
                <div class="review-head">
                    <div class="${avatarClass}">
                        ${avatarImage}
                        <span class="review-avatar-fallback">${authorInitial}</span>
                    </div>
                ...
                ${dateText ? `<p class="review-date">${escapeHtml(translations.reviews.updated_on)} ${escapeHtml(dateText)}</p>` : ''}
            </article>
        `;
    }

    function calculateReviewsStats(reviews) {
        if (!Array.isArray(reviews) || reviews.length === 0) {
            return {
                count: 0,
                averageRating: 0,
                argsRatings: [0, 0, 0, 0, 0]
            };
        }
    ...
    function buildReviewsStarsInline(rating) {
        const safeRating = Math.max(0, Math.min(5, Number(rating) || 0));
        let starsHtml = '';
        for (let i = 1; i <= 5; i++) {
            starsHtml += `<span style="color:${i <= safeRating ? '#fbbc04' : '#d7d7d7'};">&#9733;</span>`;
        }
        return starsHtml;
    }

    function updateReviewsStatsBar(reviews) {
        const statsEl = document.getElementById('google-reviews-stats-bar');
        ...
        if (!statsEl || !countEl || !avgValEl || !avgStarsEl) {
            return;
        }

        const stats = calculateReviewsStats(reviews);
        countEl.textContent = stats.count;
        avgValEl.textContent = stats.averageRating.toFixed(1);
        avgStarsEl.innerHTML = buildReviewsStarsInline(stats.averageRating);
    }

    function prioritizeReviewsByContent(reviews) {
        if (!Array.isArray(reviews)) {
            return [];
        }

        const with_comment = [];
        const without_comment = [];

        for (let i = 0; i < reviews.length; i++) {
            const review = reviews[i];
            if (review.comment && String(review.comment).trim().length > 0) {
                with_comment.push(review);
            } else {
                without_comment.push(review);
            }
        }

        return with_comment.concat(without_comment);
    }

    function renderGoogleReviews(reviews) {
        const listEl = document.getElementById('google-reviews-list');
        const statusEl = document.getElementById('google-reviews-status');
        const carouselWrapperEl = document.getElementById('google-reviews-carousel-wrapper');
        const controlsEl = document.getElementById('google-reviews-controls');
        const statsBarEl = document.getElementById('google-reviews-stats-bar');

        if (!listEl || !statusEl || !carouselWrapperEl || !controlsEl || !statsBarEl) {
            return;
        }

        googleReviewsCache = Array.isArray(reviews) ? prioritizeReviewsByContent(reviews) : [];

        if (!Array.isArray(reviews) || reviews.length === 0) {
            statusEl.style.display = 'block';
            statusEl.textContent = translations.reviews.no_reviews;
            listEl.innerHTML = '';
            carouselWrapperEl.style.display = 'none';
        ...
        carouselWrapperEl.style.display = 'block';
        statsBarEl.style.display = 'flex';
        updateReviewsStatsBar(googleReviewsCache);

        const reviewsPerSlide = getReviewsPerSlide();
        const slides = splitReviewsIntoSlides(reviews, reviewsPerSlide);

        listEl.dataset.totalSlides = String(slides.length);
        if (googleReviewsSlideIndex >= slides.length) {
            googleReviewsSlideIndex = 0;
        }

        listEl.innerHTML = slides.map(function(slideReviews) {
            return `<div class="reviews-carousel-slide">${slideReviews.map(buildReviewCard).join('')}</div>`;
        }).join('');

        controlsEl.style.display = slides.length > 1 ? 'flex' : 'none';
        attachReviewsCarouselEvents();
        updateReviewsCarouselUI();
        startReviewsAutoplay();
    }
    ...
    function loadGoogleReviews() {
        ...
        fetch('../app/api/public/get_google_reviews.php?limit=9', {
            ...
            .catch(function() {
                if (statusEl) {
                    statusEl.style.display = 'block';
                    statusEl.textContent = translations.reviews.error_loading;
                }
                stopReviewsAutoplay();
            });
    }
    ...
    window.addEventListener('resize', function() {
        ...
        reviewsResizeTimeout = setTimeout(function() {
            if (googleReviewsCache.length > 0) {
                renderGoogleReviews(googleReviewsCache);
            }
        }, 250);
    });
